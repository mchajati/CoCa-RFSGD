...
## Script purpose: Monte Carlo Simulation (mc) of 5 model parameter and the influence on Fresh Submarine Groundwater Discharge (FSGD) 
## Date:           2019/01/03
## License:        GNU GPLv3
...
MC_total = lapply(1:length(cocas), function(x) matrix(NA, ncol=14, nrow=N)) # Summary of all Monte Carlo runs
for(j in 1:length(cocas)){ names(MC_total)[j] = paste0("monte_carlo_estimates_CoCa_",cocas[j]) } # Give proper names to MC_total
...
for(n in 1:N){
  start.time  <- Sys.time()
  k           <-0
  # Load Initial Conditions from the momeva run to increase computational time
  In.cond     <- read.csv(paste0(getwd(),"/model_input/In.cond_momeva.csv")); In.cond$cat <- Co_ca_LRM$cat
  
    for(j in cocas){
      k<-k+1
      
      ### --- SET MONTE CARLO PARAMETERS --- ###
      # (1) max water volume in TOP SOIL [mm]
      repeat{     
        m      <- Co_ca_LRM$D1_average[Co_ca_LRM$cat==j]
        s      <- Co_ca_LRM$D1_stddev[Co_ca_LRM$cat==j]
        m_S1   <- log(m^2 / sqrt(s^2 + m^2))
        s_S1   <- sqrt(log(1 + (s^2 / m^2)))
        Wava   <- rlnorm(1, m_S1, s_S1)
        S1_max <- Wava 
        if(S1_max>0){break}
      }
      # (2) Depth AQUIFER [mm]
      repeat{ 
        ...
        V1tot     <- S1_max/wc1_sat                                            # TOP SOIL total Volume of [mm]
        V2tot <- 700                                                           # SUB SOIL total Volume of [mm]
        m <- 1000*Co_ca_LRM$Dtot_average[Co_ca_LRM$cat==j]
        s <- 1000*Co_ca_LRM$Dtot_stddev[Co_ca_LRM$cat==j]
        m_D3 <- log(m^2 / sqrt(s^2 + m^2))
        s_D3 <- sqrt(log(1 + (s^2 / m^2)))
        D3   <- max(1000,(rlnorm(1, m_D3, s_D3))-(V1tot + V2tot)) 
        if(D3>0){break}
      }
      # (3) saturated hydraulic conductivity TOP SOIL [mm/day]
      repeat{ 
        m_k1    <- 10*Co_ca_LRM$S1_k_average[Co_ca_LRM$cat==j]/10000  
        s_k1    <- 10*Co_ca_LRM$S1_k_stddev[Co_ca_LRM$cat==j]/10000   
        lm_k1   <- log(m_k1^2 / sqrt(s_k1^2 + m_k1^2))
        ls_k1   <- sqrt(log(1 + (s_k1^2 / m_k1^2)))
        k1        <- rlnorm(1, lm_k1, ls_k1)                        
        if(k1>0){break}
      }
      # (4) saturated hydraulic conductivity SUB SOIL [mm/day]
      repeat{
        m_k2    <- 10*Co_ca_LRM$S2_k_average[Co_ca_LRM$cat==j]/10000
        s_k2    <- 10*Co_ca_LRM$S2_k_stddev[Co_ca_LRM$cat==j]/10000  
        lm_k2   <- log(m_k2^2 / sqrt(s_k2^2 + m_k2^2))
        ls_k2   <- sqrt(log(1 + (s_k2^2 / m_k2^2)))
        k2      <- rlnorm(1, lm_k2, ls_k2)                           
        if(k2>0){break}
      }
      # (5) saturated hydraulic conductivity AQUIFER [mm/day]
      repeat{
        m_perm  <- Co_ca_LRM$S3_log_perm_avg[Co_ca_LRM$cat==j]          
        s_perm  <- Co_ca_LRM$S3_log_perm_std[Co_ca_LRM$cat==j]          
        lm_perm <- log(m_perm^2 / sqrt(s_perm^2 + m_perm^2))
        ls_perm <- sqrt(log(1 + (s_perm^2 / m_perm^2)))
        k3 <- (10^-(rlnorm(1, lm_perm, ls_perm)) * 1000 * 9.81 / 0.001) * (1000 * (60 * 60 * 24))
        if(k3>0){break}
      }
      # SET other model parameters
      S1_min    <- wc1_res*V1tot                                                # TOP SOIL min water volume in [mm]
      ...
      S2_max    <- wc2_sat*V2tot                                                # SUB SOIL max water volume in [mm]
      S2_min    <- wc2_res*V2tot                                                # SUB SOIL min water volume in [mm]
      ...
      repeat{
        ...
        In.cond$S1_0[grep(which(In.cond$cat==j) , rownames(In.cond))][1] <- S1[length(S3)]
        In.cond$S2_0[grep(which(In.cond$cat==j) , rownames(In.cond))][1] <- S2[length(S3)]
        In.cond$S3_0[grep(which(In.cond$cat==j) , rownames(In.cond))][1] <- S3[length(S3)]
        
        # INCREASE spinning up 
        diff2[i] <- S3[length(S3)] - S3[1]
        cat(paste0("  ",round(diff2[i])))
        if(abs(diff2[i])-abs(diff2[i-2])>0){ 
          x3<-max(0,x3/20)
          u<-1
          if(i>20){ 
            MC_total[[k]][n,] <- c(NA,NA,NA,NA,NA,NA,NA,NA,k1,k2,k3,S1_max,S2_max,D3)
            print("ERROR")
            break 
          }
        }else if(abs(diff2[i])<abs(diff2[i-2]*0.4)){ 
          x3<-x3 
        }else{
          x3<-x3*4^(2/u)
          u<-u+0.35
        ...
        if(abs(diff2[i]) < 20 | abs(diff2[i]) == abs(diff2[i-2])){
          
          # TEMPORARY PLOT to check model results
          par(mfrow=c(3,1))
          ...
          # SAVE results in MC_total
          MC_total[[k]][n,] <- c(sd(unlist(SGD[paste0("CoCa.",j)])),
                                 mean(unlist(SGD[paste0("CoCa.",j)])),
                                 sum(SGD[paste0("CoCa.",j)]),
                                 sum(ET_act[paste0("CoCa.",j)]),
                                 sum(OF[paste0("CoCa.",j)]),
                                 sum(cr1[paste0("CoCa.",j)]),
                                 sum(R1[paste0("CoCa.",j)]),
                                 sum(R2[paste0("CoCa.",j)]),
                                 k1,k2,k3,S1_max,S2_max,D3)
          ...
          cat(paste0(colnames("Run ",N,": ",SGD[paste0("CoCa.",j)]),": ",round(diff2[i]), "mm difference in aquifer storage (spinupfactor = ", round(x3,digits=2), "), WB = ", iszero, ", SGD_mean = ", round(mean(unlist(SGD[paste0("CoCa.",j)])),digits=2),"\n"))
          break
        }
      }
    }
...
write.csv(MC_total , file=paste0(getwd(),"/model_output/",format(Sys.Date(), '%Y%m%d'),"_",N,"runs_mc.csv"), row.names=FALSE)
save.image(paste0(getwd(),"/model_output/",format(Sys.Date(), '%Y%m%d'),"_",N,"runs_mc.RData")) 
